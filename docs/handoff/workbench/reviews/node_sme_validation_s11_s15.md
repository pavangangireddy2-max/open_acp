# Node → Cluster → Question map for SME validation — pilot sessions s11–s15  (v2)
...
>
> **How to read the evidence.** Items were grouped because *the same learners fail them together* (phi over co-attempters; robust filter N≥150, phi≥0.2, both-fail cell≥10). Clustering **proposes**; content **disposes**: `cluster` is provenance (which behaviour community found the item), `node` is the content assignment. `fail%` = first-attempt fail rate (~4.5K learners).
>
> **v2 (2026-07-14).** Full verification pass before SME handoff: **19 items moved** (each flagged `⚑ v2 MOVE` with reason — these are our corrections; please confirm them specifically) and **16 seam rows flagged** where the call is genuinely yours. Biggest change: s12's elif-ladder items (incl. all of cluster C5) moved from N2 to N3 — the elif node turns out to be behaviour-backed after all. v1 generated 2026-07-13.

## Coverage

| session | items | proposed nodes |
|---|---|---|
| s11 Conditional Statements | 75 | N1=16 · N2=16 · N3=28 · N4=15 |
| s12 Nested Conditional Statements | 24 | N1=9 · N2=7 · N3=8 |
| s13 Loops | 38 | N1=22 · N2=9 · N3=7 |
| s15 For Loop | 31 | N1=19 · N2=12 |
| s14 Understanding Coding Question Formats | 6 | N1=3 · N2=3 |

## The 19 v2 moves (confirm these first)

| session | question | moved | reason |
|---|---|---|---|
| s11 | `34be084e` | N2→N3 | boolean-ops eval, family of 49bda558/1b9dd04a |
| s11 | `7ae00d90` | N2→N3 | not(not(True)) eval, family of 0eba62cd |
| s11 | `cfab6ee6` | N2→N4 | marks sequential-if ladder, family of 0a1d0356/102d40c4 |
| s11 | `b3a8f1da` | N4→N3 | stored boolean is_day, family of 03669b6d/d4d99ccd |
| s11 | `fa634c4a` | N4→N3 | length!=breadth eval, family of 5ee2d163/27961bab |
| s11 | `c93e22f4` | N2→N1 | concept MCQ: 'indentation is optional' — indentation rule |
| s11 | `d098001b` | N3→N2 | both prints inside if, condition False, family of 1435356a |
| s12 | `37c47406` | N2→N1 | outer False → inner never reached — N1's exact demand |
| s12 | `7aabdbcb` | N2→N3 | elif %10/%5 ladder |
| s12 | `d7fe4a11` | N2→N3 | elif ladder with or-condition |
| s12 | `239a1da6` | N2→N3 | elif grade ladder (whole cluster C5 = elif ladders) |
| s12 | `823719c2` | N2→N3 | elif ladder with and-range condition (C5) |
| s12 | `5bd5e9e3` | N2→N3 | elif points ladder (C5) |
| s12 | `5158f651` | N2→N3 | elif WITHOUT condition — illegal-elif semantics |
| s12 | `99072b33` | N2→N3 | elif score ladder |
| s13 | `55fb9a8e` | N1→N2 | a reassigned True → infinite loop, family of 21ddb473 |
| s15 | `2085e437` | N1→N2 | index scan for space + slice print — clone of 385afe41/2956637e |
| s15 | `b79c7d65` | N1→N2 | for each_char in sentence + count — char iteration |
| s15 | `b9dd9ea2` | N1→N2 | for each_char in book + count — char iteration |

## Open seam questions for the SME

1. **s13 N1 vs N3** — identical `while i<=a` / `%`-filter templates sit on both sides (flagged rows). Proposed rule: trap is the `<=` boundary or zero-iteration → N3; trap is the accumulation arithmetic → N1. Confirm the rule, then place the flagged clones.
2. **s11 `dd29f7cf`** — FIB writing `if`/`else` headers: scope (N2), syntax (N1), or condition construction (N3)? Note its 76% fail is partly the FIB exact-string-match artifact.
3. **s11 C10 FIBs** (`5f1987a2`, `65a2af77`) — write-the-if-header: kept under N3 (condition construction); N1 defensible.
4. **Definitional MCQs** (`9f26c8a0` why-indentation, s13's `18da9692`/`9e9e63de` syntax/vocabulary) — keep in nearest node or park in a session-level 'vocabulary' bucket?
...
**cluster C14**
- ⚑ `458400aa` — C14 split→N1 (legal 3-space indent)
...
**cluster C15**
- ⚑ `c93e22f4` — v2 MOVE N2→N1: concept MCQ: 'indentation is optional' — indentation rule
...
**cluster C15**
- ⚑ `dd29f7cf` — seam: FIB write if/else headers — N1(syntax)/N3(condition) candidates; 76% fail partly FIB exact-match artifact
...
**cluster C6**
- ⚑ `689112ed` — C6 split→N2 (TWO-DEMAND: case-sensitive compare + trailing print)
...
**cluster C8**
- ⚑ `d098001b` — v2 MOVE N3→N2: both prints inside if, condition False, family of 1435356a
...
a comparison/expression evaluates to a bool that drives the branch; stored booleans `is_day=(t>=8)and(t<=15)`; boolean operators.  \
*28 items*

**cluster C10**
- ⚑ `5f1987a2` — C10 seam: FIB write-the-if-header — N3(condition) / N1(syntax); registry author's call
- ⚑ `65a2af77` — C10 seam: FIB write-the-if-header — N3(condition) / N1(syntax); registry author's call
...
**cluster C14**
- ⚑ `27961bab` — C14 split→N3
- ⚑ `e5036f1d` — C14 split→N3
...
**cluster C16**
- ⚑ `0eba62cd` — C16→N3 (trailing-space else is cosmetic; demand is eval)
- ⚑ `ecf8cd53` — C16→N3 (trailing-space else is cosmetic; demand is eval)
- ⚑ `b2403a5a` — C16→N3 (trailing-space else is cosmetic; demand is eval)
...
**cluster C2**
- ⚑ `28a9783a` — C2 split→N3 (stored booleans / ==,!= eval)
- ⚑ `03669b6d` — C2 split→N3 (stored booleans / ==,!= eval)
- ⚑ `8224a525` — C2 split→N3 (stored booleans / ==,!= eval)
- ⚑ `5ee2d163` — C2 split→N3 (stored booleans / ==,!= eval)
- ⚑ `7f31d394` — C2 split→N3 (stored booleans / ==,!= eval)
- ⚑ `e7aa2590` — C2 split→N3 (stored booleans / ==,!= eval)
...
**cluster C3**
- ⚑ `34be084e` — v2 MOVE N2→N3: boolean-ops eval, family of 49bda558/1b9dd04a
...
**cluster C4**
- ⚑ `b3a8f1da` — v2 MOVE N4→N3: stored boolean is_day, family of 03669b6d/d4d99ccd
- ⚑ `fa634c4a` — v2 MOVE N4→N3: length!=breadth eval, family of 5ee2d163/27961bab
...
**cluster C6**
- ⚑ `1b9dd04a` — C6 split→N3
- ⚑ `ddbd17a4` — C6 split→N3
...
**cluster C7**
- ⚑ `7ae00d90` — v2 MOVE N2→N3: not(not(True)) eval, family of 0eba62cd
...
independent sequential ifs each fire; else binds to the nearest if.  \
*15 items*
...
**cluster C2**
- ⚑ `0a1d0356` — C2 split→N4 (marks sequential-if ladder)
- ⚑ `48706095` — C2 split→N4 (marks sequential-if ladder)
- ⚑ `102d40c4` — C2 split→N4 (marks sequential-if ladder)
...
| `48706095-9ee2-4404-b102-019afbe34062` | CA_MULTIPLE_CHOICE | 9.3 | `marks = 99 /  / if marks >= 95: /     print("Satisfied") / if marks >= 80: /     print("Good") / if marks >= 40: /     print("Average") / else: /     print("F …` |
...
**cluster C3**
- ⚑ `cfab6ee6` — v2 MOVE N2→N4: marks sequential-if ladder, family of 0a1d0356/102d40c4
...
| question_id | type | fail% | code / stem |
|---|---|---|---|
| `28445134-18d2-4a2a-b0e4-cc3c108d3442` | CA_TEXTUAL | 51.6 | `marks = 70 /  / if marks > 80: /     print("Brilliant") / if marks > 65: /     print("Very good") / if marks > 50: /     print("Average") / else: /     print( …` |
...
a boolean expression gates the outer block; the inner if is never reached when the outer is False.  \
*9 items*
...
| `97adb9d2-c37b-48e2-a263-4c1ed92963f2` | CA_MULTIPLE_CHOICE | 59.8 | `expression1 = (36 < 16) / expression2 = (True or True) / if expression1: /     print("expression1 is True") /     if expression2: /         print("expression2 …` |
...
**cluster C2**
- ⚑ `fe1cb1c6` — C2 split→N1 (boolean-gated nesting)
- ⚑ `4161e7c1` — C2 split→N1 (boolean-gated nesting)
- ⚑ `fb292562` — C2 split→N1 (boolean-gated nesting)
...
**cluster C3**
- ⚑ `37c47406` — v2 MOVE N2→N1: outer False → inner never reached — N1's exact demand
...
reading multi-level nested structure: nested else binding, composite gating expressions, nested accumulation.  \
*7 items*
...
| question_id | type | fail% | code / stem |
|---|---|---|---|
| `a9cb037f-c335-4939-93eb-0f47ad653bb2` | CA_MULTIPLE_CHOICE | 42.2 | `if a: /     print("I like sports") /     if b: /         print("I like Football") /     else: /         print("I like Tennis") / else: /      print("I like re …` |

**cluster C4**

| question_id | type | fail% | code / stem |
|---|---|---|---|
| `eca0018c-5dfe-4717-ba5d-2226473e75f2` | CA_TEXTUAL | 33.5 | `a = input() / num1 = int(a[0]) / num2 = int(a[1]) / num3 = int(a[2]) / num4 = int(a[3]) /  / is_num1_smallest = ((num1 < num2) and (num1 < num3) and (num1 < n …` |
...
**cluster C6**
- ⚑ `9f26c8a0` — seam: definitional 'why indentation' MCQ — no clean node; SME decide

| question_id | type | fail% | code / stem |
|---|---|---|---|
| `2d34fbeb-acd0-427f-9b99-3773ea98e793` | CA_TEXTUAL | 37.3 | `a = input() / num1 = int(a[0]) / num2 = int(a[1]) / num3 = int(a[2]) / num4 = int(a[3]) /  / is_num1_smallest = ((num1 < num2) and (num1 < num3) and (num1 < n …` |
...
**cluster C7**
- ⚑ `7bacc79f` — seam: boolean-gated via slice/string exprs — N1/N2 border

| question_id | type | fail% | code / stem |
|---|---|---|---|
| `7bacc79f-ec67-402e-9419-28e3bc02dee5` | CA_MULTIPLE_CHOICE | 60.3 | `word_1 = "Microwave" / word_2 = "Wavelength"  / expression1 = (word_1[5:9] == word_2[:4]) / expression2 = (333 == ("3" * 3)) / if expression1: /     print("ex …` |
...
elif ladder: first-true-wins; both-false prints nothing; elif needs a condition; elif-after-else illegal.  \
*8 items*

**cluster C2**
- ⚑ `6445782f` — C2 split→N3 (elif ladder, both-false→no output)
...
**cluster C3**
- ⚑ `7aabdbcb` — v2 MOVE N2→N3: elif %10/%5 ladder

| question_id | type | fail% | code / stem |
|---|---|---|---|
| `7aabdbcb-046d-492e-82e9-3f9c1ec5abc1` | CA_MULTIPLE_CHOICE | 8.6 | `number = 20 / if number % 10 == 0: /     print("Divisible by 10") / elif number % 5 == 0: /     print("Divisible by 5") / else: /     print("Not Divisible by  …` |

**cluster C4**
- ⚑ `d7fe4a11` — v2 MOVE N2→N3: elif ladder with or-condition
...
**cluster C5**
- ⚑ `239a1da6` — v2 MOVE N2→N3: elif grade ladder (whole cluster C5 = elif ladders)
- ⚑ `823719c2` — v2 MOVE N2→N3: elif ladder with and-range condition (C5)
- ⚑ `5bd5e9e3` — v2 MOVE N2→N3: elif points ladder (C5)
...
**cluster C6**
- ⚑ `5158f651` — v2 MOVE N2→N3: elif WITHOUT condition — illegal-elif semantics
...
**cluster C7**
- ⚑ `99072b33` — v2 MOVE N2→N3: elif score ladder
...
simulate counter/accumulator to the end (sums, factorial, both-ends-moving).  \
*22 items*

**cluster C1**
- ⚑ `37b29df6` — seam N1/N3: m<=30 double %-filter clone family (see N3 defs)
...
| `37b29df6-faf1-4509-aae9-2450ba8ed46f` | CA_TEXTUAL | 44.3 | `m = 20 / sum = 0 / while m <= 30: /     if (m % 2) == 0: /         sum = (sum + 1) /     if (m % 5) == 0: /         sum = (sum + 2) /     m = (m + 1) /      / …` |
...
| `6c4bc229-ed80-4082-9713-8dfa6eae1f1d` | CA_TEXTUAL | 37.9 | `a = 1 / b = 5 / counter = 0 / sum = 0 / while counter < 4: /     sum = (sum + a + b) /     a = (a + 1) /     b = (b + 1) /     counter = counter + 1 /  / prin …` |
| `b8b6e171-bb7b-4e12-be28-87a381b96d80` | CA_MULTIPLE_CHOICE | 31.0 | `a = 5 / count = 0 / while a > 0: /     a = (a - 1) /     count = (count + a) /  / print(count)` |

**cluster C11**
- ⚑ `ef17f8d5` — seam N1/N3: `while i<=a` + %2 clone family (see 8ce95b05/f7b59138 in N3)
- ⚑ `6a1424dc` — seam N1/N3: print(n%2) clone family (see 51af25e8 in N3)
- ⚑ `ca1dd7ec` — seam N1/N3: count odds `i<=a` clone family (see bfa71718 in N3)
...
**cluster C2**
- ⚑ `e158cb7a` — seam N1/N3: m<=20 double %-filter clone family
- ⚑ `8d3e7424` — seam N1/N3: m<=30 double %-filter clone family

| question_id | type | fail% | code / stem |
|---|---|---|---|
| `e158cb7a-3341-4e51-a746-362726385798` | CA_TEXTUAL | 48.1 | `m = 1 / sum = 1 / while m <= 20: /     if (m % 2) == 0: /         sum = (sum + 1) /     if (m % 5) == 0: /         sum = (sum + 2) /     m = (m + 1) /      /  …` |
| `8d3e7424-6715-4529-b4a1-b4eeaf765d86` | CA_TEXTUAL | 45.0 | `m = 15 / sum = 0 / while m <= 30: /     if (m % 2) == 0: /         sum = (sum + 1) /     if (m % 5) == 0: /         sum = (sum + 2) /     m = (m + 1) /      / …` |
| `5d4183d9-2988-4bac-93da-2578471c0b3c` | CA_TEXTUAL | 38.5 | `a = 5 / c = 1 / sum = 0 / while c < a: /     sum = ((sum + a) + c) /     a = (a - 1) /     c = (c + 1) / print(sum)` |
| `6b70bc65-004c-4a95-9a46-0ccd9f8c6355` | CA_TEXTUAL | 35.2 | `a = 3 / b = 4 / counter = 1 / sum = 0 / while counter < 4: /     sum = (sum + a + b) /     a = (a + 1) /     b = (b + 1) /     counter = counter + 1 /  / prin …` |
...
**cluster C4**
- ⚑ `03e89465` — C4 split→N1
- ⚑ `f0043edd` — C4 split→N1 ; seam N1/N3: print(n%2) clone family (see 51af25e8 in N3)
...
**cluster C6**
- ⚑ `49d2bbaf` — seam N1/N3: count evens `i<=a` clone family (see bfa71718 in N3)
...
**cluster C7**
- ⚑ `e6f78e79` — C7 split→N1 (trace `print(n%2)`) ; seam N1/N3: print(n%2) clone family (see 51af25e8 in N3)
...
**cluster C6**
- ⚑ `55fb9a8e` — v2 MOVE N1→N2: a reassigned True → infinite loop, family of 21ddb473
...
**cluster C7**
- ⚑ `18da9692` — C7 split→N2 (loop syntax / vocabulary — definitional)
- ⚑ `9e9e63de` — C7 split→N2 (loop syntax / vocabulary — definitional)

| question_id | type | fail% | code / stem |
|---|---|---|---|
| `18da9692-5b1d-4a39-8cfb-f908d71c99ce` | MULTIPLE_CHOICE | 18.8 | `STEM: Below code snippet is the correct syntax for a while loop in Python.  ```text while condition:      code block ```` |
...
**cluster C9**
- ⚑ `8cf0f56c` — C9→N2 (all entry-condition-false → never runs)
- ⚑ `aaa1ab04` — C9→N2 (all entry-condition-false → never runs)
- ⚑ `ffb73136` — C9→N2 (all entry-condition-false → never runs)
...
`<` vs `<=` boundary, zero-iteration edges, `%` filters inside the loop.  \
*7 items*

**cluster C10**
- ⚑ `f7b59138` — seam N1/N3 (kept N3): same rule
...
| `f7cf0eaa-ede7-4189-a8ce-1423ed07a2fb` | CA_TEXTUAL | 18.5 | `a = 3 / b = 6 / counter = 0 / sum = 0 / while counter < 0: /     sum = (sum + a + b) /     a = (a + 1) /     b = (b + 1) /     counter = counter + 1 /  / prin …` |

**cluster C4**
- ⚑ `bfa71718` — C4 split→N3 (`while i<=a` + `%2` filtered sum) ; seam N1/N3 (kept N3): same rule
...
**cluster C5**
- ⚑ `8ce95b05` — seam N1/N3 (kept N3): rule = trap is <= boundary/zero-iter → N3; trap is accumulation arithmetic → N1
- ⚑ `51af25e8` — seam N1/N3 (kept N3): same rule
...
| `af480cdd-56e0-48ab-aa07-bdb98cef1e62` | CA_TEXTUAL | 39.7 | `starting_num = 10 / ending_num = 20 / total = 0 /  / for i in range(starting_num, ending_num): /     if (i % 5) == 0: /         total = total + i /          / …` |
...
| `79bf495c-51f4-4dc9-9c0e-d027a87805e6` | CA_TEXTUAL | 34.1 | `first_num = 15 / last_num = 30 / total = 0 /  / for i in range(first_num, last_num): /     if (i % 4) == 0: /         total = total + i /          / print(tot …` |
...
**cluster C4**
- ⚑ `6805d328` — C4 split→N1 (range eval-once / factorial*0)
- ⚑ `e878f4c6` — C4 split→N1 (range eval-once / factorial*0)
...
**cluster C5**
- ⚑ `3d431459` — C5 split→N1 (`range(1,n)` accumulate)
...
`for ch in s`, slices `s[k:]`/`s[:k]`, `str(n)` digits, index scans over strings.  \
*12 items*

**cluster C1**
- ⚑ `b79c7d65` — v2 MOVE N1→N2: for each_char in sentence + count — char iteration
- ⚑ `2085e437` — v2 MOVE N1→N2: index scan for space + slice print — clone of 385afe41/2956637e
- ⚑ `b9dd9ea2` — v2 MOVE N1→N2: for each_char in book + count — char iteration
...
| `2085e437-f0f8-43b3-99f3-f2bf003e680a` | CA_MULTIPLE_CHOICE | 40.7 | `sentance = "Welcome to CCBP" / length_of_sentance = len(sentance) /  / for i in range(length_of_sentance): /     if sentance[i] == " ": /         print(sentan …` |
| `b9dd9ea2-b6e1-48c7-a508-e8c091443ba3` | CA_TEXTUAL | 36.1 | `book = "Physics World" / character = "s" / count = 0 /  / for each_char in book: /     if each_char == character: /         count = count + 1 /          / pri …` |
...
**cluster C4**
- ⚑ `2956637e` — C4 split→N2 (string scan for space)
...
**cluster C5**
- ⚑ `9d7db3ce` — C5 split→N2 (slice `word[:k]` build)
- ⚑ `f991397e` — C5 split→N2 (slice `word[:k]` build)
...
| question_id | type | fail% | code / stem |
|---|---|---|---|
| `f65df76a-b2df-41f0-b7cb-4507019cabf3` | MULTIPLE_CHOICE | 19.5 | `STEM: Which of the following options is the correct form of the technical format of the given story-based question text?    **Story-Based Question Text:**   B …` |
| `f43465d6-e132-4f74-97f3-f94dc0a25da7` | MULTIPLE_CHOICE | 15.3 | `STEM: Which of the following options is the correct form of the technical format of the given story-based question text?    **Story-Based Question Text:**   L …` |
| `f01c350e-0089-401b-bee7-1fb9bcda5256` | MULTIPLE_CHOICE | 10.9 | `STEM: Which of the following options is the correct form of the technical format of the given story-based question text?    **Story-Based Question Text:**   S …` |
...
| question_id | type | fail% | code / stem |
|---|---|---|---|
| `5b83c111-4f41-4fc1-895d-5c86b05acfd5` | REARRANGE | 24.6 | `STEM: Liam wants to write a program that counts the occurrences of a specific element in a list. Convert the given code into pseudo code.    **Code:**  ```pyt …` |
...
| `d49f74d6-1fa6-4a35-b5ad-0df8e73269d8` | REARRANGE | 8.5 | `STEM: Anna is learning coding by going through different codes. Rearrange the given options in the correct order according to the given code snippet to help h …` |
